...
public class Game implements Runnable, KeyListener {
	...
	private static final int SPAWN_NEW_DEBRIS = FRAMES_PER_SECOND * 5;
		...
		for (Movable movFriend : CommandCenter.getInstance().getMovFriends()) {
			for (Movable movFoe : CommandCenter.getInstance().getMovFoes()) {
				...
				if (pntFriendCenter.distance(pntFoeCenter) < (radFriend + radFoe)) {
					...
					CommandCenter.getInstance().addScore();
					//remove the foe
		...
		if (CommandCenter.getInstance().getFalcon() != null){
			...
			for (Movable movFloater : CommandCenter.getInstance().getMovFloaters()) {
				...
				if (pntFalCenter.distance(pntFloaterCenter) < (radFalcon + radFloater)) {
					CommandCenter.getInstance().addHP();
	...
	private void spawnNewDebris() {

		//appears more often as your level increses.
		if ((System.currentTimeMillis() / ANI_DELAY) % (SPAWN_NEW_DEBRIS) == 0) {
	...
	private void checkNewLevel(){
		...
		if (isLevelClear() ){
			if (CommandCenter.getInstance().getFalcon() !=null)
				// if comes to next level, score reset
				CommandCenter.getInstance().resetScore();
				superBulletLeft = 3;
	...
	public void keyPressed(KeyEvent e) {
		...
		if (fal != null) {
			...
			case SUPER:
				if (CommandCenter.getInstance().getScore() >= 20){
					if (superBulletLeft >= 0) {
						CommandCenter.getInstance().getOpsList().enqueue(new Shotgun(fal), CollisionOp.Operation.ADD);
